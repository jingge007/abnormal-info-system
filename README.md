...
### 后端存储
- **API服务**: 项目通过代理请求访问后端API服务，包括异常报告管理、内容资料管理和文件上传等功能
- **文件存储**: 使用第三方文件存储服务，支持图片等文件的上传和管理
...
- **图片处理**：支持图片上传、编辑、预览等功能，图片自动存储到文件存储服务
...
### 4. API集成
- 无缝集成后端API服务，实现数据的存储和管理
- 通过代理配置实现前后端分离，提高系统的可维护性
...
### 6. 请求拦截优化
- 使用 axios 拦截器统一管理所有请求
- 自动根据请求方法显示不同的 loading 文案
- 统一的错误处理和超时处理机制

### 7. 全局 Loading 管理
- 实现了可配置的全局 loading 组件
- 支持自定义 loading 文案和样式
- 使用计数管理并发请求的 loading 状态

### 8. 配置集中管理
- 所有配置项集中在 `src/config/index.js` 文件中管理
- 支持 API 配置、上传配置、请求配置、环境配置
- 方便统一维护和修改
...
### 1. 统一配置
项目使用 `src/config/index.js` 文件统一管理所有配置：
...
### 2. 代理配置
项目使用 `vue.config.js` 文件配置开发服务器代理：
...
```json
{
  "rewrites": [
    {
      "source": "/api/(.*)",
      "destination": "https://unicloud-api.taicisou.cn/$1"
    }
  ]
}
```
